...
class ActionType extends Agenda
{
    ...
    public function getXML()
    {
        ...
        $xml    = $this->_createXML()->addChild($this->_namespace.':actionType',
            null, $this->_namespace($this->_namespace));
        $action = $xml->addChild($this->_namespace.':'.($this->_data['type'] == 'add/update'
                ? 'add' : $this->_data['type']));
        ...
        if ($this->_data['filter']) {
            $filter = $action->addChild('ftr:filter', null,
                $this->_namespace('ftr'));

            foreach ($this->_data['filter'] as $property => $value) {

                $ftr = $filter->addChild('ftr:'.$property,
                    is_array($value) ? null : $value);

                if (is_array($value)) {
                    foreach ($value as $tProperty => $tValue) {
                        $ftr->addChild('typ:'.$tProperty, $tValue,
                            $this->_namespace('typ'));
                    }
                }
            }
